...
public abstract class MicroService implements Runnable {
	...
	public MicroService(String name) {
		this.name = name;
		requestMap = new HashMap<Class<? extends Request>, Callback>();
		broadcastMap = new HashMap<Class<? extends Broadcast>, Callback>();
		requestOnCompleteMap = new HashMap<Request, Callback>();
	}
	
	/**
	 * map of requests keys and how to handle them
	 */
	private HashMap<Class<? extends Request>, Callback> requestMap;
	/**
	 * map of broadcasts keys and how to handle them
	 */
	private HashMap<Class<? extends Broadcast>, Callback> broadcastMap;
	/**
	 * map of requests to callback which should be called when this request has been completed.
	 */
	private HashMap<Request, Callback> requestOnCompleteMap;
	...
	protected final <R extends Request> void subscribeRequest(Class<R> type, Callback<R> callback) {
		...
		MessageBusImpl.MessageBus().subscribeRequest(type, this);
	}
	...
	protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
		...
		MessageBusImpl.MessageBus().subscribeBroadcast(type, this);
	}
	...
	protected final <T> boolean sendRequest(Request<T> r, Callback<T> onComplete) {
		if(MessageBusImpl.MessageBus().sendRequest(r, this))
		{
			requestOnCompleteMap.put(r, onComplete);
			return true;
		}
		else
			return false;
	...
	protected final void sendBroadcast(Broadcast b) {
		MessageBusImpl.MessageBus().sendBroadcast(b);
	}
	...
	protected final <T> void complete(Request<T> r, T result) {
		MessageBusImpl.MessageBus().complete(r, result);
	}
	...
	public final void run() {
		MessageBusImpl.MessageBus().register(this);
		initialize();
		while (!terminated) 
		{
			try {
				Message message = MessageBusImpl.MessageBus().awaitMessage(this);
				
				//Checks whether the message is Request, Broadcast or ReqComplete and lunch call.
				if (Request.class.isAssignableFrom(message.getClass()))
					requestMap.get(message.getClass()).call(message);
				else if (Broadcast.class.isAssignableFrom(message.getClass()))
					broadcastMap.get(message.getClass()).call(message);
				else if (RequestCompleted.class.isAssignableFrom(message.getClass()))
					requestOnCompleteMap.get(((RequestCompleted) message).getCompletedRequest()).call(((RequestCompleted) message).getResult());

			} 
			catch (IllegalStateException e) { throw new IllegalStateException(getName() + " was never registered.");} 
			catch (InterruptedException e) { terminated = true;}
		}
		MessageBusImpl.MessageBus().unregister(this);
	}
